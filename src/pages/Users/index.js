...
class UserSearch extends Component {
  ...
  formatDataForTable = (
    ...
    keys.map(key => {
      ...
      formatedUsers.push([
        ...
        0,
        0,
        <IconButton onClick={() => this.handleTrashClick(user[1])}>
          <FontAwesomeIcon
            icon={faTrash}
            key={`${user.id}-trash`}
            color="white"
  ...
  handleSearchFieldChange = event => {
    ...
    if (event.target.value == "") {
      ...
      let usersFiltered = users.filter(user => {
        if (user[0].includes(searchField) || user[1].includes(searchField)) {
          return user;
        }
      });
  ...
  render() {
    const { usersFiltered, columns, searchField } = this.state;
    ...
    return (
      <Grid container className={classes.externalContainer} direction="column">
        ...
        <Grid item>
          ...
          />
          <DataTable tableData={usersFiltered} tableHeader={columns} />
        </Grid>
      </Grid>
    );
  }
